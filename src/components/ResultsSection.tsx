      ...
      <div ref={sectionRef} className="container mx-auto px-4 md:px-8 opacity-0">
        ...
        <div className="relative bg-[#0E141F] rounded-2xl shadow-lg p-6 md:p-10 overflow-hidden">
          <div className="absolute top-6 right-6 flex space-x-2">
            <button onClick={slideLeft} disabled={position === 0} className={`p-2 rounded-full ${position === 0 ? 'bg-gray-600 text-gray-400' : 'bg-nexblue text-white'}`} aria-label="Slide anterior">
              <ArrowLeft size={20} />
            </button>
            <button onClick={slideRight} disabled={position === beforeAfterContent.length - 1} className={`p-2 rounded-full ${position === beforeAfterContent.length - 1 ? 'bg-gray-600 text-gray-400' : 'bg-nexblue text-white'}`} aria-label="Próximo slide">
              <ArrowRight size={20} />
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-[#1A1F2C] rounded-xl p-6 shadow-md border border-red-300/20 transition-all duration-500">
              ...
              <ul className="space-y-4">
                {content.before.points.map((point, idx) => <li key={idx} className="flex items-start">
            ...
            <div className="bg-[#1A1F2C] rounded-xl p-6 shadow-md border border-green-300/20 transition-all duration-500">
              ...
              <ul className="space-y-4">
                {content.after.points.map((point, idx) => <li key={idx} className="flex items-start">
        ...
        <div className="mt-10 text-center">
          ...
          <a href="#contato" className="inline-flex items-center bg-nexorange hover:bg-nexorange/90 text-white px-8 py-4 rounded-lg font-semibold text-lg transition-all duration-300 hover:shadow-lg">
